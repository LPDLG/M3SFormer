...
def inference_inpainting(local_rank=0, args=None):
    # 1. 加载模型
    info = get_model(args=args, model_name=args.name)
    model = info['model'].to('cuda').eval()
    epoch = info['epoch']
    model_name = info['model_name']

    # 2. 准备数据
    data = ImagePathDataset(
    ...
    # 3. 创建保存目录
    save_root = os.path.join('RESULT', f'{model_name}_e{epoch}', args.save_dir)
    os.makedirs(save_root, exist_ok=True)
    ...
    for i in range(len(data)):
        data_i = data[i]
        print(f"处理 {i+1}/{len(data)}: {data_i['relative_path']}")

        # 准备输入数据
        img = data_i['image'].unsqueeze(0).to('cuda')  # 添加batch维度
        mask = data_i['mask'].unsqueeze(0).to('cuda') if 'mask' in data_i else None
        ...
        with torch.no_grad():
            ...
            tokens = model.get_tokens(
                img, 
                mask=mask,
                return_token_index=True
            )
            
            # 解码阶段 (自动处理掩码区域)
            reconstructed = model.decode(
                tokens['token'],
                mask_im=img if mask is not None else None,
                mask=mask,
                combine_rec_and_gt=True  # 合并修复和原始区域
            )

        # 6. 保存结果
        base_name = os.path.basename(data_i['relative_path'])
        save_path = os.path.join(save_root, base_name)
        
        # 转换并保存图像
        rec_img = reconstructed.squeeze(0).permute(1, 2, 0).cpu().numpy()
        cv2.imwrite(save_path, cv2.cvtColor(rec_img, cv2.COLOR_RGB2BGR))

        # 可选：保存掩码图像
        if args.save_masked_image and mask is not None:
            masked_img = (img * mask).squeeze(0).permute(1, 2, 0).cpu().numpy()
            mask_save_path = os.path.join(save_root, f"masked_{base_name}")
            cv2.imwrite(mask_save_path, cv2.cvtColor(masked_img, cv2.COLOR_RGB2BGR))
