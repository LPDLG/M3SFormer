...
from collections.abc import Iterable  # 添加这行导入
from re import M
import torch
...
from image_synthesis.modeling.utils.misc import distributed_sinkhorn, get_token_type, pixel_shuffle
...
class VectorQuantizer(nn.Module):
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        return z_q, min_encoding_indices, d
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        z_q, min_encoding_indices, distance = self._quantize(z_flattened, token_type=token_type_flattened, topk=topk, step=step, total_steps=total_steps)
        ...
        if self.training:
            ...
            loss = torch.torch.tensor(0.0).to(z_q)
        ...
        output = {
            ...
            'distance': distance.view(batch_size, height, width, distance.shape[-1])
        }
    ...
    @staticmethod
    def count_flops(m, input, output):
        # import pdb; pdb.set_trace()
        flops = 0
        b, c ,h, w = input[0].shape

        n_e = m.masked_embed_start
        dim = m.e_dim

        # power 2
        flops = flops + (h*w*dim) + (n_e*dim + dim)

        # sum dim 2
        flops = flops + h*w*(dim-1) + n_e*(dim-1)

        # dot production
        flops = flops + h*w*dim*n_e + h*w*n_e*(dim-1)
...
class UpSample(nn.Module):
    ...
    def count_flops(m, x, y):
        ...
        if m.activate_before == 'relu':
            m.total_ops += calc_func.calculate_relu(x[0].numel())

        if m.upsample_type == 'deconv':
            m.total_ops += m.deconv.total_ops
        elif m.upsample_type in ['bilinear', 'nearest']:
            y_ = F.interpolate(x[0], scale_factor=2.0, mode=m.upsample_type)
            m.total_ops += calc_func.calculate_upsample(mode=m.upsample_type, output_size=y_.numel())
            m.total_ops += m.conv.total_ops
        if m.activate_after == 'relu':
            m.total_ops += calc_func.calculate_relu(y.numel())
...
class DownSample(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.downsample_type != 'conv':
            if self.downsample_type in ['nearest', 'bilinear']:
                x = F.interpolate(x, scale_factor=0.5, mode=self.downsample_type)
    ...
    def count_flops(m, x, y):
        ...
        if m.downsample_type != 'conv':
            if m.downsample_type in ['nearest', 'bilinear']:
                y_ = F.interpolate(x[0], scale_factor=0.5, mode=m.downsample_type)
                m.total_ops += calc_func.calculate_upsample(mode=m.downsample_type, output_size=y_.numel())
            elif M.downsample_type == 'maxpool':
                pass 
            elif m.downsample_type == 'avgpool':
                y_ = torch.avg_pool2d(x[0], kernel_size=2, stride=2, padding=0, dilation=1)
                m.total_ops += calc_func.calculate_avgpool(y_.numel())
        m.total_ops += m.conv.total_ops
...
class PatchDecoder2(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.out_dim = out_ch * stride * stride 
        self.stride = stride 
        self.out_ch = out_ch
        self.in_ch = in_ch
        self.in_channels = in_ch

        assert up_layer_with_image is False, "For patch level decoders, how can be add refference branch?"
        self.requires_image = up_layer_with_image 
        
        if num_pre_layers == 0:
            self.pre_layers = nn.Identity()
        elif num_pre_layers == 1:
            self.pre_layers = nn.Sequential(*[
                nn.ReLU(inplace=True),
                nn.Linear(in_ch, res_ch),
            ])
        else:
            raise NotImplementedError('Following the configurations in PatchEncoder2')
    ...
    def forward(self, x, mask=None):
        ...
        b, c, h, w = x.shape
        x = x.view(b, c, -1).permute(0, 2, 1) # B x HW x C
        
        x = self.pre_layers(x)
        x = self.res_layers(x)
        x = self.post_layer(x) # B x HW x 3SS

        x = x.permute(0, 2, 1) # B x 3SS x HW
        x = torch.nn.functional.fold(x, output_size=(h, w), kernel_size=(1,1), stride=(1,1)) # B x 3SS x H x W
        x  = pixel_shuffle(x, out_size=(int(self.stride*h), int(self.stride*w))) # B x 3 x HS x WS
        return x
...
class PatchVQGAN(BaseCodec):
    ...
    def multi_pixels_with_mask(self, data, mask, mask_pixel_value=None):
        if self.im_process_info['mean'].sum() != 0.0:
            ...
            fill_image = torch.zeros_like(data_m)
            if mask_pixel_value is not None:
                fill_image += mask_pixel_value
            data_m[~mask] = ((fill_image - self.im_process_info['mean'].to(data_m.device)) / self.im_process_info['std'].to(data_m.device))[~mask]
        else:
            data_m = data * mask.to(data)
            if mask_pixel_value is not None:
                fill_image = torch.zeros_like(data_m) + mask_pixel_value
                data_m[~mask] = ((fill_image - self.im_process_info['mean'].to(data_m.device)) / self.im_process_info['std'].to(data_m.device))[~mask]
        return data_m
    ...
    def get_features(self, data, mask=None, 
                     ...
                     mask_pixel_value=None # the pixel value to fill the masked regions
        ):
        """
        ...
        if return_quantize_feature or return_token:
            if mask is not None:
                token_type, _ = get_token_type(mask, token_shape, type=self.resize_mask_type) # B x 1 x H x W
            else:
            ...
            if return_token:
                output['token'] = quant_out['index'] # B x H x W
            if return_distance:
                output['distance'] = quant_out['distance'] # B x H x W x N
    ...
    def get_tokens(self, data, mask=None, erase_mask=None, topk=1, return_token_index=False, cache=True, **kwargs):
        ...
        if erase_mask is not None:
            token_type_erase, _ = get_token_type(erase_mask, token_shape, type=self.resize_mask_type) # B x 1 x H x W
        else:
    ...
    def sample(self, batch):
        ...
        if 'erase_mask' in batch:
            token_type_erase = get_token_type(batch['erase_mask'], token_shape, type=self.resize_mask_type)[0].to(self.device) # B x 1 x H x W
        else:
            token_type_erase = torch.ones((x.shape[0], 1, x.shape[2], x.shape[3])).long().to(self.device)
    ...
    def parameters(self, recurse=True, name=None):
        ...
        param_groups = {
            ...
            'discriminator': [self.loss.discriminator] if hasattr(self.loss, 'discriminator') else [],
            'flow_optimizer': [self._flow_wrapper] if hasattr(self, '_flow_wrapper') else []
        }

        # 验证参数组名称
        if name not in param_groups:
            raise ValueError(f"Invalid parameter group: {name}. "
                        f"Available: {list(param_groups.keys())}")

        # 收集参数
        params = []
        for module in param_groups[name]:
    ...
    def forward(self, batch, name='none', return_loss=True, step=0, total_steps=None, **kwargs):
        ...
        # 统一输出结构
        output = {
            'loss': torch.tensor(0.0, device=self.device),  # 默认loss
            'reconstruction': None
        }

        # 输入验证
        if name not in ['generator', 'discriminator', 'none', 'reconstruction']:
            raise ValueError(f"Invalid forward mode: {name}. Expected 'generator'/'discriminator'/'none'/'reconstruction'")
        ...
        try:
            ...
            if name == 'none':
                ...
                token_type = get_token_type(batch.get('mask', torch.ones_like(input[:,:1])), 
                                        token_shape, self.resize_mask_type)[0].to(self.device)
                
                quant_out = self.quantize(x, token_type=token_type)
                quant = self.post_quant_conv(quant_out['quantize'])
                
                # 解码
                if self.decoder.requires_image and 'mask' in batch:
                    masked_input = self.multi_pixels_with_mask(input, batch['mask'])
                    rec = self.decoder(quant, masked_input, mask=batch['mask'])
                else:
                    rec = self.decoder(quant)
                
                output.update({
                    'reconstruction': rec,
                    'quantize_loss': quant_out.get('quantize_loss', torch.tensor(0.0))
                })
                return output
            ...
            # 生成器模式
                ...
                token_type = get_token_type(batch.get('erase_mask', torch.ones_like(input[:,:1])), 
                                        token_shape, self.resize_mask_type)[0].to(self.device)

                quant_out = self.quantize(x, token_type=token_type, step=step, total_steps=total_steps)
                quant = self.post_quant_conv(quant_out['quantize'])
                
                # 保存临时变量供判别器使用
                self.input_tmp = input
                if self.decoder.requires_image:
                    self.rec_tmp = self.decoder(quant, self.multi_pixels_with_mask(input, batch['mask']), 
                                            mask=batch['mask'])
                else:
                    self.rec_tmp = self.decoder(quant)

                # 计算损失
                if isinstance(self.loss, VQLPIPSWithDiscriminator):
                    loss_output = self.loss(
                        codebook_loss=quant_out['quantize_loss'],
                        inputs=input,
                    ...
                    other_loss = {k: v for k, v in quant_out.items() if 'loss' in k}
                    
                    loss_im = (self.post_process(input) / 255.0) if self.loss.norm_to_0_1 else input
                    loss_rec = (self.post_process(self.rec_tmp) / 255.0) if self.loss.norm_to_0_1 else self.rec_tmp
                    ...
                    raise NotImplementedError(f'Unsupported loss type: {type(self.loss)}')

                output.update({
                    'loss': loss_output['loss'],
                    'reconstruction': self.rec_tmp,
                    **{k: v for k, v in quant_out.items() if v.numel() == 1 and len(v.shape) == 0 and k != 'loss'}
                })

            # 判别器模式
            elif name == 'discriminator':
                if not hasattr(self, 'input_tmp') or not hasattr(self, 'rec_tmp'):
                    raise RuntimeError("必须先运行generator前向传播才能运行discriminator")
                ...
                if isinstance(self.loss, VQLPIPSWithDiscriminator):
                    ...
                    loss_im = (self.post_process(self.input_tmp) / 255.0) if self.loss.norm_to_0_1 else self.input_tmp
                    loss_rec = (self.post_process(self.rec_tmp) / 255.0) if self.loss.norm_to_0_1 else self.rec_tmp
            ...
            # 重建模式（用于采样）
                ...
                sample_output = self.sample(batch)
                output.update({
                    'reconstruction': sample_output['reconstruction'],
                    'loss': torch.tensor(0.0, device=self.device)
                })

        except Exception as e:
            print(f"Error in forward pass (name={name}): {str(e)}")
            raise

        # 确保返回的loss是需要梯度的
        if output['loss'].requires_grad is False and phase == 'train':
            output['loss'] = output['loss'].clone().requires_grad_(True)

        if 'loss' in output and output['loss'].requires_grad is False and phase == 'train':
            output['loss'] = output['loss'].clone().requires_grad_(True)
