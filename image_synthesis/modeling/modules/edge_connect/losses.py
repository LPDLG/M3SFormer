...
class EdgeConnectLoss(nn.Module):
    def __init__(self,
        ...
        self.g_color_loss_weight = g_color_loss_weight
    ...
    def forward(self, 
        ...
        if name == 'generator':
            ...
            if self.g_color_loss_weight > 0 and step >= self.color_start:
                color_loss = self._color_consistency_loss(
                    reconstruction, 
                    image,
                    mask.float() if mask is not None else 1.0
                ) * self.g_color_loss_weight
                out['color_loss'] = color_loss
            out['loss'] = gen_loss
